...

{
    ...
    //	   O R D E R S
    // Create an Orders menu to add to the Menu Bar
    ...
    list_sweets_button->set_tooltip_markup("List a sweet");
...
{
	Gtk::MessageDialog getSweet(*this, "Please choose a sweet to purchase");
	for(int i = 0; i<_store->num_sweets(); ++i)
	{
		button = Mainwin::button;
		button->set_label(_store->sweet(i).name());
		button->show();
		
		getSweet.get_vbox()->pack_start(*button);
	}
	getSweet.run();

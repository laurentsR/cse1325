...
Mainwin::Mainwin() : shelter{new Shelter{"Pawsome Pals"}} {
    ...
    // L O A D
    // Icon for open
    Gtk::Image* open = Gtk::manage(new Gtk::Image{"load.png"});
    open_button = Gtk::manage(new Gtk::ToolButton(*open));
    open_button->set_tooltip_markup("Load Existing Data");
...
{
	Gtk::Dialog mdialog{"Save Progress?", *this};
	mdialog.set_default_size(300, 100);
	Gtk::Label blank{""};
	Gtk::Label label{"Would you like to save your progress?"};
	mdialog.get_content_area()->add(blank);
	mdialog.get_content_area()->add(label);
	mdialog.add_button("Save", 1);
    mdialog.add_button("Discard Changes", 0);
	mdialog.show_all();
...
void Mainwin::on_save_click() {
    try {
        ...
        std::ostringstream oss;
        oss << "Unable to save file\n" << e.what();
        Gtk::MessageDialog{*this, oss.str(), false, Gtk::MESSAGE_ERROR}.run();
    }
}

void Mainwin::on_save_as_click() {
    Gtk::FileChooserDialog dialog("Please choose a file name to save as",
...
void Mainwin::on_open_click() {
    ...
    Gtk::FileChooserDialog dialog("Please choose a file to load data from",
...
void Mainwin::on_about_click() {
    Glib::ustring s = "Pawsome Pals " + VERSION
        + "\nCopyright 2019 by Ryan J. Laurents\n\n"

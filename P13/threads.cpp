...
class Prime_numbers {
  public:
    ...
    void find_primes(int lower, int upper) {
    	...
    	int j = 0;
    	int each = (upper-lower)/NUM_THREADS;
    	for (j; j < NUM_THREADS; j++)
    	{
    		t[j] = std::thread(&Prime_numbers::find_primes_thread, this, (lower + (each * j)), (each * (j+1)));
    	}
    ...
    void find_primes_thread(int lower, int upper) {
        ...
        for (int i=lower; i<=upper; ++i) {
            if (Prime_numbers::is_prime(i)) {
            	m.lock();
                Prime_numbers::primes.push_back(i);
                m.unlock();
            }
        }
    }
